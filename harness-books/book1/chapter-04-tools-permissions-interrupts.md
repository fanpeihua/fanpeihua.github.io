...
| 用户插话 + `interruptBehavior=cancel` | 工具尚未完成 | user interrupt | 取消执行，生成 `user interrupted` synthetic result |
| 用户插话 + `interruptBehavior=block` | 工具尚未完成 | user interrupt | 继续工具至自然终点，阻塞新消息 |
| streaming fallback | 批次 queued/executing | fallback | discard 整批，按顺序 yield fallback synthetic result |
| abort 时有 pending `tool_use` | 任意 | abort signal | 补齐 synthetic `tool_result`，账本闭环 |
