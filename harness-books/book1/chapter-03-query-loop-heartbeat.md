<!-- 渲染版本：/harness-books/book1/chapter-03-query-loop-heartbeat.html -->
<!-- Rendered HTML: /harness-books/book1/chapter-03-query-loop-heartbeat.html -->
...
assert hasAttemptedReactiveCompact ⇒ no further compact      # 防自回环
```
...
| 事件顺序 | 前置状态 | 触发 | 下一步 |
|---|---|---|---|
| stream 正常结束 + `tool_use` | 有未完成 tool_use | stop reason | follow-up，进入工具执行 |
| stream 正常结束，无 `tool_use` | 无 pending tool | stop reason | 进入 stop hooks |
...
| stop hook block + PTL 再现 | `hasAttemptedReactiveCompact` | double failure | 跳过 stop hooks，surface error |
