<!-- 渲染版本：/harness-books/book1/chapter-02-prompt-is-control-plane.html -->
<!-- Rendered HTML: /harness-books/book1/chapter-02-prompt-is-control-plane.html -->

# 第 2 章 Prompt 不是人格，Prompt 是控制平面

## 2.1 把 prompt 当成人设，是一种常见误会

一说起 system prompt，很多人首先想到的是一段熟悉的话术：你是谁、你擅长什么、最好再有一点稳定人格。对聊天系统这种理解够用，对要读文件、调工具、动 shell、跨轮执行的代理系统则明显不够。人设解决的是“它像什么”，控制平面解决的是“它能做什么、什么时候做、做错了怎么办、谁来兜底”。两者不在同一层。

Claude Code 的实现就说明了这一点：它的 system prompt 是一组分层拼装的行为区块，更接近一套运行时协议，而不是一篇人物小传。
...
这里的 proactive mode 处理更直白：`src/utils/systemPrompt.ts:99` 往后，如果 agent prompt 和 proactive mode 同时存在，agent prompt 不再替换默认 prompt，而是附加在其后。通用制度可以叠加岗位说明书，但不能被岗位说明书直接冲掉。

### 骨架：prompt 装配的优先级链 (skeleton)
...
assert exists unique base in sources                # 基线必须唯一
assert precedence(override) > precedence(default)   # 顺序硬编码，不靠"后写为准"
assert appendSystemPrompt never replaces base       # 追加只能附加
```

这三条不变式一旦任意一条被破坏，prompt 就会退化成一块谁后写谁说了算的涂鸦板。
